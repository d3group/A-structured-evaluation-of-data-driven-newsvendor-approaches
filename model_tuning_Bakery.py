...
def main():
    ...
    # load data
    X = pd.read_csv("data/bakery_data.csv", index_col=0)
    y = pd.read_csv("data/bakery_target.csv", index_col=0)
    
    X.drop(columns=['date', 'is_schoolholiday', 'is_holiday', 'is_holiday_next2days', 'promotion_currentweek', 'promotion_lastweek', 'rain', 'temperature'], inplace=True)
